# Governed revenue-cycle workflow

## Scope

The new endpoint is `/api/governed-revenue-cycle`. It implements traceable eligibility, encounter, coding, claim, remittance, denial, appeal, payment, and reconciliation state. The state sequence is:

`eligibility_pending → encounter_verified → coding_review → claim_ready → submitted → remittance_received → denied/appealed or paid → reconciled`
...
- Case identity, evidence, and events are protected from destructive updates/deletes by database constraints and triggers. The event history is readable at `GET /api/governed-revenue-cycle/cases/:id/history`.
- Deterministic triage validates eligibility, encounter completeness, code/coverage versions, claim control reference, receipts, remittance and payment digests. It always returns `automatedDecision: false` and `requiresHumanReview: true`.
- No payer, clearinghouse, EHR, licensed code set, or payment credential is supplied. Certified staff approve coding and no autonomous submission occurs.
