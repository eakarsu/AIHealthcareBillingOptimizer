...
// Feature slug: gap-no-notification-engine-0-references
...
router.post('/', auth, (req, res) => {
  ...
  if (aiRateLimiter) return aiRateLimiter(req, res, () => handle('gap-no-notification-engine-0-references', req, res));
  return handle('gap-no-notification-engine-0-references', req, res);
});
