...
export default function GapNoFileUploadForClinicalNotes() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-file-upload-for-clinical-notes', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: '2rem', maxWidth: '900px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '1.75rem', marginBottom: '0.5rem' }}>No file upload for clinical notes</h1>
      <p style={{ color: '#666', marginBottom: '1.5rem' }}>No file upload for clinical notes</p>
      <form onSubmit={handleSubmit} style={{ marginBottom: '1.5rem' }}>
        ...
        >
          {loading ? 'Processing...' : 'Run No file upload for clinical notes'}
        </button>
      </form>
